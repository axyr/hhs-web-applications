...
const pages = computed(() =>
  Array.from({ length: numberOfPages.value }, (_, i) => i + 1)
);
...
const firstItem = computed(
  () => props.perPage * props.page - props.perPage + 1
);
...
<template>
  <div class="pagination">
      ...
      <div>
        <select id="per-page" class="shadow rounded" @change="changePerPage">
          <option
            ...
            :value="perPageOption"
            :key="perPageOption"
            :selected="perPageOption === props.perPage"
          >
            {{ perPageOption }}
          </option>
        </select>
    ...
    <div>
      <ul>
        <li>
          <button :disabled="!showGoFirst" @click="goToFirstPage">
            &laquo;
          </button>
        </li>
        <li>
          <button :disabled="!showGoPrevious" @click="goToPreviousPage">
            &lsaquo;
          </button>
        </li>
        <li v-for="page in pages" :key="page">
          <button :disabled="page === props.page" @click="goToPage(page)">
            {{ page }}
          </button>
        </li>
        <li>
          <button :disabled="!showGoNext" @click="goToNextPage">
            &rsaquo;
          </button>
        </li>
        <li>
