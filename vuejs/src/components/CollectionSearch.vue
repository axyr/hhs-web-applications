...
function toggleCategorySearch(category) {
    if (props.search.categories.includes(category.id)) {
        props.search.categories = props.search.categories.filter(id => id !== category.id);
    } else {
...
<template>
    <div>
        <div id="filterbar">
            <div id="search">
                <input type="search" id="search-field" class="rounded shadow" placeholder="Search..." v-model="search.term" @keyup="applySearch(300)" @search="applySearch">
            </div>
            <div id="favorites">
                <div class="checkbox shadow rounded bg-white">
                    <label for="checkbox-favorite">
                        <input type="checkbox" value="favorite" id="checkbox-favorite" v-model="search.favorites" @change="applySearch">
                        {{ favoritesLabel }}
                    </label>
                </div>
            </div>
        ...
        <div id="categories">
            <div v-for="category in categories" class="checkbox shadow rounded bg-white">
                <label :for="`checkbox-category-${category.id}`">
                    <input type="checkbox" value="favorite" :id="`checkbox-category-${category.id}`" :checked="categoryIsSelected(category)" @change="toggleCategorySearch(category)">
